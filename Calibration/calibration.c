...
void order_phases(GPIOStruct *gpio, ControllerStruct *controller, PreferenceWriter *prefs)
{
    ...
    {                                                       //rotate for 2 electrical cycles
        ...
        theta_actual = PositionSensor_GetMechPositionFixed();
        if(theta_ref==0)
        {
    ...
    float theta_end = PositionSensor_GetMechPositionFixed();
...
void calibrate(GPIOStruct *gpio, ControllerStruct *controller, PreferenceWriter *prefs)
{
    ...
    ///Set voltage angle to zero, wait for rotor position to settle
    abc(theta_ref, v_d, v_q, &v_u, &v_v, &v_w);             // inverse dq0 transform on voltages(d,q转换为三相电压)
    svm(1.0, v_u, v_v, v_w, &dtc_u, &dtc_v, &dtc_w);        // space vector modulation(svpwm)
    ...
    {                                                   // rotate forwards
        ...
        raw_f[i] = PositionSensor_GetRawPosition();
        printf("%.4f   %.4f   %.4f   %d\n\r", theta_ref/(NPP), theta_actual, error_f[i], raw_f[i]);
        //theta_ref += delta;
    }
    ...
    {                                                   // rotate backwards
        ...
        PositionSensor_Sample(DT);                                     // sample position sensor
        theta_actual = PositionSensor_GetMechPositionFixed();          // get mechanical position
        error_b[i] = theta_ref/NPP - theta_actual;
        raw_b[i] = PositionSensor_GetRawPosition();
        printf("%.4f   %.4f   %.4f   %d\n\r", theta_ref/(NPP), theta_actual, error_b[i], raw_b[i]);
        //theta_ref -= delta;
    }    
    ...
    PositionSensor_SetElecOffset(offset);                                              // Set position sensor offset
    ...
    /* 对error[n]进行FIR滤波，得到error_filt[n] */
    for (int i = 0; i<n; i++)
    {
    ...
    {                                          // build lookup table
        ...
        lut[ind] = (int) ((error_filt[i*NPP] - mean)*(float)(PositionSensor_GetCPR())/(2.0f*PI));
        printf("%d   %d   %d\n\r", i, ind, lut[ind]);
        wait(.001);
    }
    ...
    memcpy(&ENCODER_LUT, lut, n_lut*4);                                 // copy the lookup table to the flash array
